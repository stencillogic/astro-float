...
impl Mantissa {
    ...
    fn fft_reverse(parts: &mut [SliceWithSign], w: usize, k1: usize) {
        if k1 == 2 {
            ...
            Self::fft_butterfly(&mut p1[0], &mut p2[0]);
            for j in 1..k2 {
                let (a, b) = (&mut p1[j], &mut p2[j]);
                let w_shift = w * (k1 - j);
                b.shift_left(w_shift);
                Self::fft_butterfly(a, b);
            }
    ...
    fn fft_compute_chunks<'a>(num: &mut SliceWithSign, n: usize, tmp_buf: &'a mut [Digit]) -> SliceWithSign<'a> {
        let mut sign = 1;
        tmp_buf.fill(0);
    ...
    fn fft_mul(d1: &[Digit], d2: &[Digit], d3: &mut [Digit]) -> Result<(), Error> {
        let l = (d1.len() + d2.len())*DIGIT_BIT_SIZE;
        let (n, k1, k, m, n1, t) = Self::fft_params(l);
        let w = t*2;
        let part_len = (m + n*2 + DIGIT_BIT_SIZE - 1) / DIGIT_BIT_SIZE;
        //println!("{:?}", (n, k1, k, m, n1, t, w));

        let mut buf = DigitBuf::new(4*k1*part_len + 5*part_len)?;
        buf.fill(0);
...
/* 
        ...
        Self::fft_reverse(&mut parts2, w, k1);
        for v in &parts2 {
            let (q, r) = Self::div_basic(v, &[k1 as Digit]).unwrap();
            let (q, r) = Self::div_basic(&q, &modulus[..3]).unwrap();
            println!("{} * {:?}", v.sign(), r);
        } */
        ...
        for (j, part3) in parts3.iter_mut().enumerate() {
            ...
            let mut out = SliceWithSign::new_mut(&mut d3[(j*m)/DIGIT_BIT_SIZE..], 1);
            part3.shift_left((j*m) % DIGIT_BIT_SIZE);
...
mod tests {
    ...
    fn test_fft_mul() {
        ...
        let mut s1 = [11, 12, 13, 14, 15, 16, 17, 18];
        let mut s2 = [21, 22, 23, 24, 25, 26, 27, 28];
        let mut ret_s = [0; 16];
        let mut ref_s = [0; 16];
    ...
    fn fft_mul_perf() {
        ...
        for _ in 0..5 {
            ...
            for mut ni in n.drain(..) {
                Mantissa::fft_mul(&mut ni, &f, &mut ret).unwrap();
            }
