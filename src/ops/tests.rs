...
use crate::defs::{RoundingMode, EXPONENT_MAX, EXPONENT_MIN};
...
fn test_sin_asin() {
    ...
    for _ in 0..1000 {
        ...
        if ONE
            .sub(&d2.abs().unwrap(), RoundingMode::None)
            .unwrap()
            .cmp(&thres)
            >= 0
        {
