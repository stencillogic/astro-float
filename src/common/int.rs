...
impl Int {
    ...
    pub fn from_usize(mut u: usize) -> Result<Self, Error> {
    ...
    pub fn add(&self, x: &Self) -> Result<Self, Error> {

        let mut buf = WordBuf::new(self.buf.len().max(x.buf.len()) + 1)?;

        let x1 = SliceWithSign::new(&self.buf, self.sign);
        let x2 = SliceWithSign::new(&x.buf, x.sign);
        let mut x3 = SliceWithSign::new_mut(&mut buf ,1);
    ...
    pub fn mul(&self, x: &Self) -> Result<Self, Error> {

        let mut buf = WordBuf::new(self.buf.len() + x.buf.len())?;

        let x1 = SliceWithSign::new(&self.buf, 1);
        let x2 = SliceWithSign::new(&x.buf, 1);
        let mut x3 = SliceWithSign::new_mut(&mut buf , 1);
    ...
    pub fn bit_len(&self) -> usize {
        ...
        l * WORD_BIT_SIZE + shift
    }

    pub fn as_float(&self) -> Result<BigFloatNumber, Error> {
        let n = self.bit_len();
        let sign = if self.sign < 0 { Sign::Neg } else { Sign::Pos };
        BigFloatNumber::from_raw_parts(&self.buf, n, sign, self.buf.len() as Exponent)
    }
